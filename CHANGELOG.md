...
- Added GitHub Actions workflow (`.github/workflows/publish-npm.yml`) to publish to npm on `v*` tags with provenance after a successful CI run (requires `NPM_TOKEN` and workflow `id-token: write`; npm 2FA settings must allow tokens, or use npm Trusted Publisher/OIDC).
